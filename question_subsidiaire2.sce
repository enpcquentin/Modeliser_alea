...
xtitle('Loi empirique des données et loi de mélange obtenue pour 3 classes');
...
proba_appartenir_3 = [];
for i=1:n
	ftheta =  R(4,N+1) * normale(y(i),R(1,N+1),R(6,N+1)) + R(5,N+1) * normale(y(i),R(2,N+1),R(7,N+1)) + (1 - R(4,N+1) + R(5,N+1)) * normale(y(i),R(3,N+1),R(8,N+1)) ;
	proba_appartenir_1 = [proba_appartenir_1 , R(4,N+1) * normale(y(i),R(1,N+1),R(6,N+1)) / ftheta ] ;
	proba_appartenir_2 = [proba_appartenir_2 , R(5,N+1) * normale(y(i),R(2,N+1),R(7,N+1)) / ftheta ] ;
	proba_appartenir_3 = [proba_appartenir_3 , (1 - R(4,N+1) + R(5,N+1)) * normale(y(i),R(3,N+1),R(8,N+1)) / ftheta ] ;
end;
plot2d(y,proba_appartenir_1,style = 1);
plot2d(y,proba_appartenir_2,style = 2);
plot2d(y,proba_appartenir_3,style = 3);
legends(['Probabilité d appartenir à la classe 1 sachant le ratio','Probabilité d appartenir à la classe 2 sachant le ratio','Probabilité d appartenir à la classe 3 sachant le ratio'],[1,2,3],'ur') ;
